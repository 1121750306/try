...
#### html33
 > word-break 和 word-wrap 以及 white-space
 ```
 ...
 .text-gradient {
  ...
  // 设置背景色
  -webkit-background-clip: text; // 设置背景色所在区域
  -webkit-text-fill-color: transparent // 设置文本颜色，该属性会覆盖color
}
 ```
...
```javascript
 ...
    var a = {n: 1};
    var b = a;
    a.x = a = {n: 2}  
    此处在执行该行前，就已经将a等指向了具体的引用，即a.x和a在执行之前都是a的引用{n:1}
    所以此处相当于 a = {n: 2}; {n: 1}.x = a;
